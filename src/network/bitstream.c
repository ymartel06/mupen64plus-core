...
int BitStream_write_char_array(BIT_STREAM *bstream, char *content, int size)
{
    ...
    int res = check_bitstream_size(bstream, size);
    if (res == 0)
    {
        return res;
    }
    ...
    memcpy(bstream->data + bstream->length, content, size);
...
char* BitStream_read_char_array(BIT_STREAM *bstream, int size)
{
    ...
    memcpy(buffer, bstream->data + bstream->read_index, size);
    bstream->read_index += size;
    return buffer;
}
